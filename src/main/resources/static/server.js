...
/*
		...
		fs1.readFile("randomdata.json", {encoding: 'utf-8'}, function(err, data){
			...
			*/
			var fs = require('fs');
			fs.writeFile("src/main/resources/static/I1.json", JSON.stringify(I.slice(0, I.length/2)), function(err) {
				if(err) {
					return console.log(err);
				}
				console.log("The file was saved!");
			});
			fs.writeFile("src/main/resources/static/I2.json", JSON.stringify(I.slice(I.length/2)), function(err) {
				if(err) {
					return console.log(err);
				}
				console.log("The file was saved!");
			...
			fs.writeFile("src/main/resources/static/J.json", JSON.stringify(J), function(err) {
				if(err) {
					return console.log(err);
				}
				console.log("The file was saved!");
